int gameloop(int n,int m,int turn,int Size,char game[Size][Size],char name1[],int namelen1,char name2[],int namelen2,int score1,int score2,int moves1,int moves2,int remMoves,int copygame[remMoves][7],int redogame[remMoves][8],int moves,int redomoves,int scorearray[])
{
    ...
    int invalid=0,u=1,r=0,x=1; //variables (help us if there undo or redo moves can be played or not) and (invalid for invalid inputs)
    ...
    while(playing)   //Game Loop
    {
        ...
        else if((row==2) && (col==2))    //save
        {
            ...
            if((fileNum==1) || (fileNum==2) || (fileNum==3))
            {
               ...
               // fwrite(scorearray,sizeof(int),2,save);
                ...
                printf(BYEL"\n   Game saved to %s"reset,whichfile);
                char enter;
